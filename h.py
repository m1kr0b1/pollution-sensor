    ...
    try:
        with serial.Serial(args.device, baudrate=9600) as ser:
            ...
            if args.url:
                ...
                r = requests.post(args.url, data={"pm10": pm10, "pm2.5": pm25, "lat": readGPS().get('latitude'), "lon": readGPS().get('longitude'), "alt": readGPS().get('speed'), "spd": readGPS().get('altitude2'), "time": readGPS().get('timeUtc'), "satt": readGPS().get('satellites')})
                logging.debug(r)
