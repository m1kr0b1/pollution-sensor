...
def readGPS():
  ...
  return {'gpsSignal':gpsSignal, 'latitude':latitude, 'longitude':longitude, 'timeUtc':timeUtc, 'timeFix':timeFix, 'altitude2':altitude2, 'eps':eps, 'epx':epx, 'epv':epv, 'ept':ept, 'speed':speed, 'climb':climb, 'tra
ck':track, 'mode':mode, 'satellites':satellites}
        ...
        with serial.Serial(args.device, baudrate=9600) as ser:
	        ...
	        while(True):
	            ...
	            if args.url:
					...
					postdata = {"pm10": pm10, "pm2.5": pm25, "lat": gps_data.get('latitude'), "lon": gps_data.get('longitude'), "alt": gps_data.get('speed'), "spd": gps_data.get('altitude2'), "sat
t": gps_data.get('satellites')}
